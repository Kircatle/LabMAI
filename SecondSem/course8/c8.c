...
bool list_insert_elem(list *l, int i, int t) //вставка элемента в любую часть списка
{
...
void list_exchange(list *l, int k) //обмен двумя элементами                                             
{
    int t;
    t=list_fetch(l, list_prev_elem(l,k));
    list_store(l, list_prev_elem(l,k), list_fetch(l, list_next_elem(l,k)));
    list_store(l, list_next_elem(l,k), t);
}
