...
int list_update(list *l, int size)  //обновление длины списка(если был добавлен какой-либо элемет или удалён, то используется эта функция для обновления связей и длины списка)
{
    int c=0;
...
bool list_insert_elem(list *l, int i, char t) //вставка элемента в любую часть списка(не эффективно, но просто вставляем элемент в список, на указанное место, а остальное копируется вправо)
{
...
bool list_delete_elem(list* l, int i) //Удаление элемента из списка (весь список копируется влево на этот элемент)
{
    if (i>list_size(l))
    {
        printf("\nЭлемента с таким индексном не существует!\n");
